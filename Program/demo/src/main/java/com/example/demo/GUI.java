...
public class GUI {
    ...
    private ArrayList<String> recordGameMoves;
    ...
    public void makeBoard() {
        ...
        for (int i = 0; i < board.getBoardSize(); i++) {
            for (int j = 0; j < board.getBoardSize(); j++) {
                Box box2 = new Box();
                recordGameBoard.addBox(box2, i, j);
            }
        }
        ...
        for (int i = 0; i < board.getBoardSize(); i++) {
            for (int j = 0; j < board.getBoardSize(); j++) {
                Box box = new Box(board, redPlayer, bluePlayer, displayTurn, SOSBoard, this,
                        sgGame, ggGame, recordGameBoard, recordGameMoves);
        ...
        if(board.getIsBluePlayerComputer() && board.getIsSimpleGame()) {
            sgGame.gameRules(board, SOSBoard, bluePlayer, redPlayer, this,
                                board.getBoxOfBoard(0,0), displayTurn, recordGameBoard,
                                recordGameMoves);
        }
        ...
        btn.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent actionEvent) {
                ...
                recordGameBoard.setBoardSize((board.getBoardSize()));
                resetGUI();
            }
        });
    }
    ...
    public void resetGUI() {
        ...
        boolean isRecordGameOn = recordBox.isSelected();
        ...
        board.setRecordGame(isRecordGameOn);
        SOSBoard.resetBoard();
        SOSBoard.initBoard();
        ...
        recordGameBoard.resetBoard();
        recordGameBoard.initBoard();
        recordGameBoard.setIsSimpleGame(isSGSelected);
        recordGameBoard.setRedPlayerComputer(isRedComputer);
        recordGameBoard.setBluePlayerComputer(isBlueComputer);
