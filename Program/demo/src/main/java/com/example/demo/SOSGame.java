...
public class SOSGame {
    ...
    private ArrayList recordGameMoves;
    ...
    SOSGame() {
        ...
        recordGameMoves = new ArrayList<String>();
    }
    ...
    public boolean drawALetter(Board board, Board SOSBoard, PlayerBox blue, PlayerBox red,
                               ...
                               Board recordBoard, ArrayList<String> recordMoves) {
        this.board = board;
        ...
        recordGameBoard = recordBoard;
        recordGameMoves = recordMoves;
            ...
            if(this.board.getIsBlueTurn()) {
                ...
                for(int i = 0; i < this.board.getBoardSize(); ++i) {
                    for(int j = 0; j < this.board.getBoardSize(); ++j) {
                        if(this.board.getBoxOfBoard(i,j).getValue() !=
                           recordGameBoard.getBoxOfBoard(i,j).getValue()) {
                            recordGameMoves.add("Blue Player placed a " +
                                    this.board.getBoxOfBoard(i,j).getValue() + " at row: " +
                                    i + " and column: " + j + System.lineSeparator());
                            if(this.board.getBoxOfBoard(i,j).getValue() == "S") {
                                recordGameBoard.getBoxOfBoard(i, j).drawS();
                            }
                            else {
                                recordGameBoard.getBoxOfBoard(i,j).drawO();
                            }
                        }
                    }
                }
            ...
            //Else it is Red Player's turn.
                ...
                for(int i = 0; i < this.board.getBoardSize(); ++i) {
                    for(int j = 0; j < this.board.getBoardSize(); ++j) {
                        if(this.board.getBoxOfBoard(i,j).getValue() !=
                           recordGameBoard.getBoxOfBoard(i,j).getValue()) {
                                recordGameMoves.add("Red Player placed a " +
                                    this.board.getBoxOfBoard(i,j).getValue() + " at row: " +
                                    i + " and column: " + j + "\n");
    ...
    public Board getRecordGameBoard() {return recordGameBoard;}
    public ArrayList<String> getRecordGameMoves() {return recordGameMoves;}
    ...
    public void setRecordGameBoard (Board input) {recordGameBoard = input;}
    public void setRecordGameMoves (ArrayList<String> input) {recordGameMoves = input;}
}